...
static void example_print_scan_ssid(uint32_t index, const char *ssid)
{
    char ssid_buf[WIFI_MAX_SSID_LEN] = {0};
    size_t ssid_len;
    ...
    ssid_len = strnlen(ssid, WIFI_MAX_SSID_LEN - 1);
    if (memcpy_s(ssid_buf, sizeof(ssid_buf), ssid, ssid_len) != EOK) {
        PRINT("AP[%u]: <copy failed>\r\n", index);
        return;
    }
    ssid_buf[ssid_len] = '\0';
    PRINT("AP[%u]: %s\r\n", index, ssid_buf);
                                           ...
                                           wifi_sta_config_stru *expected_bss)
{
    ...
    PRINT("Looking for AP: %s, scan count: %u\r\n", expected_ssid, num);

    /* 筛选扫描到的Wi-Fi网络，选择待连接的网络 */
    for (bss_index = 0; bss_index < num; bss_index++) {
        size_t expected_len = strlen(expected_ssid);
        size_t scan_ssid_len = strnlen(result[bss_index].ssid, WIFI_MAX_SSID_LEN);

        example_print_scan_ssid(bss_index, result[bss_index].ssid);
        if (expected_len == scan_ssid_len) {
            if (memcmp(expected_ssid, result[bss_index].ssid, expected_len) == 0) {
                break;
            }
        }
    }
